...
import loon.android.AndroidAssets;
...
public class TCPImageDecoder {
    ...
    public TCPImageDecoder(String filename) throws Exception {
        this((InputStream) new ByteArrayInputStream(LSystem.base().assets().getBytesSync(filename)));


    }

    public TCPImageDecoder(InputStream in) throws Exception {
    ...
    public List<TCPFrame> loadFrames(int startIndex, int endIndex) throws IllegalStateException, IOException {
        ...
        for (int i = 0; i < frameInfos.size(); i++) {
            ...
            if (!rawFrame) {
                ...
                texture.update(tex);
                frames.add(new TCPFrame(texture, frameInfo.refX, frameInfo.refY, frameInfo.delay));
            } else {
...
// public void setColorSections(Section[] sections) {
    ...
    public void loadColorationProfile(String filename) throws Exception{
        InputStream is = (InputStream) new ByteArrayInputStream( LSystem.base().assets().getBytesSync(filename));
        if (is != null) {
