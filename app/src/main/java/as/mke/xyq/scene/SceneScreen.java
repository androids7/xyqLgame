...
public class SceneScreen extends Screen {
    ...
    final static private LFont fpsFont = LFont
            .getFont(LSystem.getSystemGameFontName(), 0, 20);
    final static private LFont dialogueFont = LFont
            .getFont(LSystem.getSystemGameFontName(), 0, 16);
...
//        searcher = new AStar();
    ...
    protected void createMainWinUI() {
       LTexture heroframe = SpriteFactory.loadAnimationAsBitmap("wzife/main/heroframe.tcp", 0)[0];
        LTexture heroHeader = SpriteFactory.loadAnimationAsBitmap("wzife/photo/facesmall/0010.tcp", 0)[0];
        LTexture blank = SpriteFactory.loadAnimationAsBitmap("wzife/main/blank.tcp", 0)[0];
        LTexture hp = SpriteFactory.loadAnimationAsBitmap("wzife/main/hp.tcp", 0)[0];
        LTexture mp = SpriteFactory.loadAnimationAsBitmap("wzife/main/mp.tcp", 0)[0];
        LTexture exp = SpriteFactory.loadAnimationAsBitmap("wzife/main/exp.tcp", 0)[0];

        LPaper heroframeComp = new LPaper(heroframe, 522, 0);
      //  LImage blankImg = new LImage(blank);
        LPaper[] blankComps = new LPaper[4];
        blankComps[0] = new LPaper(blank,572,0);
        blankComps[1] = new LPaper(blank,572,12);
        blankComps[2] = new LPaper(blank,572,24);
        blankComps[3] = new LPaper(blank,572,36);
        LPaper hpComp = new LPaper(hp,585,3);
        LPaper mpComp = new LPaper(mp,585,15);
        LPaper expComp = new LPaper(exp,585,39);
     //   LImage headerImage = new LImage(heroHeader);
        LTexture[] headerImages = new LTexture[]{heroHeader,heroHeader,heroHeader};
        LButton heroHeaderComp = new LButton("",heroHeader.getWidth(),heroHeader.getHeight(), 526, 3){
            public void doClick() {
                getHero().stop(true);
        ...
        heroHeaderComp.setImages(heroHeader);
        add(heroframeComp);
        add(heroHeaderComp);
        add(blankComps[0]);
        add(blankComps[1]);
    ...
    protected void drawNpcDialogue(GLEx g) {
        /*
    ...
    private void drawClickEffect(GLEx g) {
        if(clicking){
            Point p = this.localToView(clickLocation);
            clickEffect.draw(g, p.x, p.y);
        }
    }

    protected void drawHero(GLEx g) {
        try {
            if(hero != null){
    ...
    protected void drawSceneBrand(GLEx g) {
...
*/
    ...
    public void touchDown(GameTouch e) {
        ...
        int x = (int)e.getX();
        //如果点击在UI组件上则不处理
        if(this.getTopComponent().contains(x,y) != false){
            return;
        }
...
*/
    ...
    private byte[] loadBlock(String filename) {
        ...
        try {
            InputStream in =(InputStream) new ByteArrayInputStream(LSystem.base().assets().getBytesSync(filename));
